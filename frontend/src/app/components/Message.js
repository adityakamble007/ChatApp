  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        ...
        const messageResponse = await fetch(`http://localhost:20670/messages/${user.id}/${recipientId}`);
        const messagesData = await messageResponse.json();
  
        // Fetch recipient details from backend
        const recipientResponse = await fetch(`http://localhost:20670/user/${recipientId}`);
        const recipientData = await recipientResponse.json();
  
        // Fetch your own user details from backend
        const userResponse = await fetch(`http://localhost:20670/user/${user.id}`);
         ...
         {messages.map((msg, index) => (
          ...
          <div key={index} className="flex justify-end mb-4 cursor-pointer">
            ...
            <div className="w-9 h-9 rounded-full flex items-center justify-center ml-2">
              <img
               src={msg.image} alt="My Avatar" className="w-8 h-8 rounded-full" />
            </div>
          </div>
          ) : (
          <div key={index} className="flex mb-4 cursor-pointer">
            <div className="w-9 h-9 rounded-full flex items-center justify-center mr-2">
              <img
               src={msg.image} alt="User Avatar" className="w-8 h-8 rounded-full" />
            </div>
      ...
      <div className="bg-white border-t border-gray-300 p-4 absolute bottom-0 w-3/4 z-10">
        <div className="flex items-center">
          ...
          <button
            ...
            className={`bg-indigo-500 text-white px-4 py-2 rounded-md ml-2 ${!message.trim() ? "opacity-50 cursor-not-allowed" : ""}`}
          >
            Send
          </button>
        </div>
